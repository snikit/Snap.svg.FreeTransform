(function() {
  ...
  Snap.plugin(function(Snap, Element, Paper, global) {
    ...
    Element.prototype.ftCreateHandles = function() {
      ...
      var rotateDragger = this.paper
        .circle(
          bb.cx + bb.width + ftOption.handleLength,
          bb.cy,
          ftOption.handleRadius
        )
        .attr({ fill: ftOption.handleFill });
      var translateDragger = this.paper
        .circle(bb.cx, bb.cy, ftOption.handleRadius)
        .attr({ fill: ftOption.handleFill });
      ...
      freetransEl.data(
        ...
        calcDistance(
          bb.cx,
          bb.cy,
          rotateDragger.attr('cx'),
          rotateDragger.attr('cy')
        )
      );
    ...
    Element.prototype.ftDrawJoinLine = function(handle) {
      ...
      if (this.data('joinLine')) {
        ...
        return this.paper
          .line(thisBB.cx, thisBB.cy, handle.attr('cx'), handle.attr('cy'))
          .attr(lineAttributes);
      }
  ...
  function dragHandleRotateMove(mainEl, dx, dy, x, y, event) {
    ...
    mainEl.data(
      'angle',
      Snap.angle(mainBB.cx, mainBB.cy, handle.attr('cx'), handle.attr('cy')) -
        180
    );
